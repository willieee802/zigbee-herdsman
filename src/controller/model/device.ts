...
class Device extends Entity {
  ...
  set powerSource(powerSource: string) {
    this._powerSource =
      typeof powerSource === 'number'
        ? Zcl.PowerSource[powerSource & ~(1 << 7)]
        : powerSource;
  }
  ...
  get skipTimeResponse(): boolean {
    return this._skipTimeResponse;
  }
  set skipTimeResponse(skipTimeResponse: boolean) {
    this._skipTimeResponse = skipTimeResponse;
  }
  get defaultSendRequestWhen(): SendRequestWhen {
    return this._defaultSendRequestWhen;
  }
  ...
  private static devices: Map<number, { [ieeeAddr: string]: Device }> = new Map<
    number,
    { [ieeeAddr: string]: Device }
  >();
  ...
  private constructor(
    ...
    meta: KeyValue,
    lastSeen: number,
    defaultSendRequestWhen: SendRequestWhen,
  ...
  public createEndpoint(ID: number): Endpoint {
    if (this.getEndpoint(ID)) {
      throw new Error(
        `Device '${this.ieeeAddr}' already has an endpoint '${ID}'`
      );
    }
  ...
  public async onZclData(
    dataPayload: AdapterEvents.ZclDataPayload,
    endpoint: Endpoint
  ): Promise<void> {
    ...
    if (
      ...
      (frame.isCommand('readRsp') || frame.isCommand('report'))
    ) {
      for (const [key, value] of Object.entries(
        ZclFrameConverter.attributeKeyValue(frame)
      )) {
    ...
    if (frame.isGlobal() && frame.isCommand('read')) {
      ...
      if (
        frame.Cluster.name in attributes &&
        (frame.Cluster.name !== 'genTime' || !this._skipTimeResponse)
      ) {
    ...
    if (
      frame.isSpecific() &&
      frame.isCluster('genPollCtrl') &&
      frame.isCommand('checkin')
    ) {
      try {
        if (this.hasPendingRequests() || this._checkinInterval === undefined) {
          ...
          await endpoint.command(frame.Cluster.ID, 'checkinRsp', payload, {
            sendWhen: 'immediate',
          });
          ...
          await Promise.all(
            this.endpoints.map(async (e) => e.sendPendingRequests(true))
          );
    ...
    const isDefaultResponse =
      frame.isGlobal() && frame.getCommand().name === 'defaultRsp';
    const commandHasResponse = frame.getCommand().hasOwnProperty('response');
    const disableDefaultResponse =
      frame.Header.frameControl.disableDefaultResponse;
  ...
  private static fromDatabaseEntry(
    ...
    const endpoints = Object.values(entry.endpoints).map((e): Endpoint => {
      return Endpoint.fromDatabaseRecord(
        e,
        networkAddress,
        ieeeAddr,
        databaseID
    ...
    if (defaultSendRequestWhen == null) {
      ...
      if (
        entry.hasOwnProperty('useImplicitCheckin') &&
        !entry.useImplicitCheckin
      ) {
        defaultSendRequestWhen = 'fastpoll';
      } else if (
        entry.hasOwnProperty('defaultSendWhenActive') &&
    ...
    if (
      endpoints.filter((e): boolean => e.supportsInputCluster('genPollCtrl'))
        .length > 0
    ) {
    ...
    debug.log(
      `Request Queue (${ieeeAddr}): default expiration timeout set to ${pendingRequestTimeout}`
    );
  ...
  private toDatabaseEntry(): DatabaseEntry {
    ...
    return {
      ...
      defaultSendRequestWhen: this.defaultSendRequestWhen,
      checkinInterval: this.checkinInterval,
    };
  }

  public save(writeDatabase = true): void {
    Entity.getDatabaseByID(this.databaseID).update(
      this.toDatabaseEntry(),
      writeDatabase
    );
  }
  ...
  public static byIeeeAddr(
    databaseID: number,
    ieeeAddr: string,
    includeDeleted = false
  ): Device {
    Device.loadFromDatabaseIfNecessary();
    const device: Device = Device.devices.get(databaseID)[ieeeAddr];
    return device?._deleted && !includeDeleted ? undefined : device;
  }

  public static byNetworkAddress(
    networkAddress: number,
    databaseID: number
  ): Device {
    return Device.allByDatabaseID(databaseID).find(
      (d) => d.networkAddress === networkAddress
    );
  }

  public static byType(type: DeviceType, databaseID: number): Device[] {
  ...
  public static allByDatabaseID(databaseID: number): Device[] {
    Device.loadFromDatabaseIfNecessary();
    return Object.values(Device.devices.get(databaseID)).filter(
      (d) => !d._deleted
    );
  }
  ...
  public static create(
    ...
    if (
      Device.devices.get(databaseID)[ieeeAddr] &&
      !Device.devices.get(databaseID)[ieeeAddr]._deleted
    ) {
  ...
  public async interview(): Promise<void> {
    ...
    try {
      ...
      if (this.interviewQuirks()) {
        debug.log(
          `Interview - completed for device '${this.ieeeAddr}' because of quirks ('${e}')`
        );
      } else {
        debug.log(
          `Interview - failed for device '${this.ieeeAddr}' with error '${e.stack}'`
        );
        error = e;
      }
    } finally {
  ...
  private interviewQuirks(): boolean {
    debug.log(
      `Interview - quirks check for '${this.modelID}'-'${this.manufacturerName}'-'${this.type}'`
    );
    ...
    if (
      this.modelID?.match('^TS\\d*$') &&
      (this.manufacturerName?.match('^_TZ.*_.*$') ||
    ...
    const match = Object.keys(lookup).find(
      (key) => this.modelID && this.modelID.match(key)
    );
    if (match) {
      const info = lookup[match];
      debug.log(
        `Interview procedure failed but got modelID matching '${match}', assuming interview succeeded`
      );
  ...
  private async interviewInternal(): Promise<void> {
    const nodeDescriptorQuery = async (): Promise<void> => {
      const nodeDescriptor = await Entity.getAdapterByID(
        this.databaseID
      ).nodeDescriptor(this.networkAddress);
      this._manufacturerID = nodeDescriptor.manufacturerCode;
      this._type = nodeDescriptor.type;
      debug.log(
        `Interview - got node descriptor for device '${this.ieeeAddr}'`
      );
    };

    const hasNodeDescriptor = (): boolean =>
      this._manufacturerID != null && this._type != null;
    ...
    if (!hasNodeDescriptor()) {
      for (let attempt = 0; attempt < 6; attempt++) {
        try {
          ...
          if (this.interviewQuirks()) {
            debug.log(
              `Interview - completed for device '${this.ieeeAddr}' because of quirks ('${error}')`
            );
            return;
          } else {
            // Most of the times the first node descriptor query fails and the seconds one succeeds.
            debug.log(
              `Interview - node descriptor request failed for '${
                this.ieeeAddr
              }', attempt ${attempt + 1}`
            );
          }
      ...
      debug.log(
        `Interview - skip node descriptor request for '${this.ieeeAddr}', already got it`
      );
    }

    if (!hasNodeDescriptor()) {
      throw new Error(
        `Interview failed because can not get node descriptor ('${this.ieeeAddr}')`
      );
    }
    ...
    if (this.manufacturerID === 4619 && this._type === 'EndDevice') {
      ...
      debug.log(
        'Interview - Detected potential TuYa end device, reading modelID and manufacturerName...'
      );
      try {
        const endpoint = Endpoint.create(
          this.databaseID,
        ...
        Object.entries(result).forEach((entry) =>
          Device.ReportablePropertiesMapping[entry[0]].set(entry[1], this)
        );
      } catch (error) {
        /* istanbul ignore next */
        debug.log(
          `Interview - TuYa read modelID and manufacturerName failed (${error})`
        );
      }
    }
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        debug.log(
          `Interview - active endpoints request failed for '${
            this.ieeeAddr
          }', attempt ${attempt + 1}`
        );
      }
    ...
    if (!activeEndpoints) {
      throw new Error(
        `Interview failed because can not get active endpoints ('${this.ieeeAddr}')`
      );
    }
    ...
    for (const endpointID of activeEndpoints.endpoints.filter((e) => e !== 0)) {
      ...
      const simpleDescriptor = await adapter.simpleDescriptor(
        this.networkAddress,
        endpoint.ID
      );
      ...
      debug.log(
        `Interview - got simple descriptor for endpoint '${endpoint.ID}' device '${this.ieeeAddr}'`
      );
            ...
            try {
              ...
              try {
                ...
                if (item.key === 'modelID' || item.key === 'manufacturerName') {
                  debug.log(
                    `Interview - first ${item.key} retrieval attempt failed, ` +
              ...
              debug.log(
                `Interview - got '${item.key}' for device '${this.ieeeAddr}'`
              );
            } catch (error) {
              debug.log(
    ...
    // Enroll IAS device
    for (const endpoint of this.endpoints.filter((e): boolean =>
      e.supportsInputCluster('ssIasZone')
    )) {
      debug.log(
        `Interview - IAS - enrolling '${this.ieeeAddr}' endpoint '${endpoint.ID}'`
      );

      const stateBefore = await endpoint.read(
        'ssIasZone',
        ['iasCieAddr', 'zoneState'],
        { sendWhen: 'immediate' }
      );
      debug.log(
        `Interview - IAS - before enrolling state: '${JSON.stringify(
          stateBefore
        )}'`
      );
      ...
      if (
        stateBefore.zoneState !== 1 ||
        stateBefore.iasCieAddr !== coordinator.ieeeAddr
      ) {
        debug.log(`Interview - IAS - not enrolled, enrolling`);

        await endpoint.write(
          'ssIasZone',
          { iasCieAddr: coordinator.ieeeAddr },
          { sendWhen: 'immediate' }
        );
        ...
        await Wait(500);
        debug.log(
          `IAS - '${this.ieeeAddr}' sending enroll response (auto enroll)`
        );
        const payload = { enrollrspcode: 0, zoneid: 23 };
        await endpoint.command('ssIasZone', 'enrollRsp', payload, {
          disableDefaultResponse: true,
        ...
        for (let attempt = 0; attempt < 20; attempt++) {
          ...
          debug.log(
            `Interview - IAS - after enrolling state (${attempt}): '${JSON.stringify(
              stateAfter
            )}'`
          );
        ...
        if (enrolled) {
          debug.log(
            `Interview - IAS successfully enrolled '${this.ieeeAddr}' endpoint '${endpoint.ID}'`
          );
        } else {
          throw new Error(
    ...
    try {
      for (const endpoint of this.endpoints.filter((e): boolean =>
        e.supportsInputCluster('genPollCtrl')
      )) {
        debug.log(
          `Interview - Poll control - binding '${this.ieeeAddr}' endpoint '${endpoint.ID}'`
        );
        ...
        this._checkinInterval = pollPeriod.checkinInterval / 4; // convert to seconds
        this.pendingRequestTimeout = this._checkinInterval * 1000; // milliseconds
        if (pollPeriod.checkinInterval <= 2400) {
          // 10 minutes
  ...
  public async removeFromNetwork(): Promise<void> {
    if (this._type === 'GreenPower') {
      ...
      await Entity.getAdapterByID(this.databaseID).sendZclFrameToAll(
        242,
        frame,
        242
      );
    } else
      await Entity.getAdapterByID(this.databaseID).removeDevice(
        this.networkAddress,
        this.ieeeAddr
      );
  ...
  public async routingTable(): Promise<RoutingTable> {
    return Entity.getAdapterByID(this.databaseID).routingTable(
      this.networkAddress
    );
  }
